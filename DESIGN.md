...
textAssets/
├── Scenes/
│   ├── CursedPortal.unity  [Manual setup per M1/M18]
│   └── OtherDimension.unity
├── Scripts/
│   ├── Core/          [M2,M4,M17,M19,M24]
│   ├── AI/            [M3,M9,M11,M14,M16,M21]
│   ├── AudioVFX/      [M5,M12,M13,M15,M20,M22,M23]
│   ├── UI/            [M7,M10]
│   ├── Props/         [M8]
│   └── Editor/        [M10,M25 automation]
├── Prefabs/
│   ├── Managers/
│   └── Props/
├── PostFX/
│   └── Profiles/      [Manual Volume assets]
├── StreamingAssets/
│   ├── PoeStories/    [User TXT]
│   └── SpiritProfiles/
│       └── poe_spirits.json
└── cursed_portal_build.yaml
🔧 JSON: poe_spirits.json (M9)
JSON{
  "Raven": {
    "story": "raven.txt",
...
🧬 Core Scripts (M1-M2,M4,M17,M19,M24)
...
🤖 AI Scripts (M3,M9,M11,M14,M16,M21)
...
🔊 AudioVFX Scripts (M5,M12,M13,M15,M20,M22,M23)
...
🛠️ Editor Scripts (Automation M10,M25)
...
public static class SceneWiringTool {
    ...
    public static void WireScene() {
        ...
        Debug.Log("🔗 Wired! (Singletons auto)");
    }
}
#endif
📋 PostFX (M6 - Manual Setup)

Create Global Volume GameObject.
Add Volume component → New Profile.
...
🏗️ Scene Hierarchies & Positions (Drag Prefabs In)
CursedPortal.unity (M1, 10x10x5m Room)
textCursedPortal (Empty)
├── PlayerRig (Capsule collider, Rigidbody)
│   └── Main Camera (Cinemachine POV, Pos: 0,1.8,-8 | Rot:0,0,0 | InteractionManager)
├── RoomRoot (Cube walls/floor, scale accordingly)
│   ├── Table (Prefab @ 0,0.5,0)
│   ├── CrystalBall (Prefab @ 0,1.2,0)
│   ├── Booth (Prefab @ 5,0,5)
│   └── Mirror (Prefab @ -5,2,5 | Plane facing player)
├── Managers (Prefab)
├── Environment
│   ├── FogParticles (ParticleSystem Hemisphere)
│   └── GlobalVolume (PostFX)
└── UI
    ├── ChatCanvas (UIChat prefab)
    └── DebugCanvas (Slider + Text)
...
├── Platform (Plane scale 8x1, cracked mat)
├── SpiritCore (Sphere glow @ 0,3,2 | DimensionalLight + EpilogueNarrator)
├── Environment (Mist particles, Skybox HDRI)
├── Managers (Prefab, DontDestroyOnLoad)
└── UIEpilogue (Canvas)
⚙️ Build Manifest: cursed_portal_build.yaml (M25)
YAMLproject_name: "Cursed Portal – Poe Parlor"
...
Interactions: E on props → LLM chats → Spook++.
Ramp: Fog/audio/VFX escalate 0-5.
Portal: Lvl5 → Fade → OtherDimension.
Epilogue: LLM farewell w/ memory → E exit.
Persistence: Memory saves/loads across sessions.
Debug: F-keys, slider work.
Modular: Disable M11+ for base demo.

Extend: User TXT drop → JSON add → New spirits. Ritual complete! 👻🚀
